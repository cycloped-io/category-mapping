...
options = Slop.new do
  banner "#{$PROGRAM_NAME} -f mapping.csv -o results.csv\n" +
    "Export results of disambiguation with local support/heuristics"
...
CSV.open(options[:output],"w") do |output|
  ...
  mapping.each.with_progress do |(category_name,mapped_name),candidates|
    if candidates.size >= 1
      ...
      best = best.first
      if options[:bijective]
        reverse_map[best.cyc_name] << [category_name,best]
      else
  ...
  if options[:bijective]
    reverse_map.each do |cyc_name,categories|
      if categories.size == 1
        category_name,best = categories.first
      else
        category_name,best = categories.sort_by{|c,_| -JaroWinkler.r_distance(cyc_name,c,ignore_case: true) }.first
      end
      output << [category_name,best.cyc_id,best.cyc_name,best.probability]
    end
  end
