...
options = Slop.new do
  ...
  on :t=, :threshold, 'Probability threshold', default: 0.0, as: Float
end
...
cyc_names = {}
2.times do |index|
  mappings << Cyclopedio::Mapping::BidirectionalMap.new
  ...
  CSV.open(file) do |input|
    input.with_progress do |row|
      ...
      row.each_slice(3) do |cyc_id, cyc_name, probability|
        next if !probability.nil? && probability.to_f<threshold
        #p cyc_id, cyc_name, category_name
...
Kernel.puts("| %.2f | %.1f%% | %.1f%% | %.1f%% | %.1f%% |" % [threshold, precision, soft_precision, recall, f1])
